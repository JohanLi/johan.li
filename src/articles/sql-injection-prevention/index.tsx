  ...
  return (
    <ArticleLayout metadata={metadata}>
      <P>
        ...
        has been and still is SQL injection. It has led to countless stolen
        login credentials, credit card numbers, compromised websites, and denial
        of service attacks.
      </P>
      <P>
        Fortunately, preventing SQL injection is straightforward, using{' '}
        <strong>parameterized queries</strong>. But what is it about
        parameterized queries that keep you safe? Similarly, using{' '}
      ...
      <P>
        Typically, we’d use a driver (also called a client library or adapter)
        to interface with a database. We write an SQL statement and pass it to a
        query() or execute() method provided by that driver. The driver, in
        turn, communicates with the database using the protocol of whichever
        DBMS we’re using.
      </P>
      <Image
      ...
      <P>
        ...
        statement to it and substitute the values of our original statement with
      ...
      <P>
        ...
        prepared statements as part of their protocol. For PostgreSQL, instead
        of sending just a <i>Query</i> message, the query happens over three
      ...
      <P>
        ...
        the burden is on the driver to do it properly – something that’s hard to
        verify as users of that driver. However, this shouldn’t be a concern as
        long as we are using known and mature drivers.
      </P>
      <P>
        On the flipside, prepared statements do have a minor cost in that they
        require more round trips to the database. For the vast majority of
      ...
      <P>
        If we want dynamic SQL statements, a solution is to form the dynamic
        parts using a list of allowed values (e.g., ASC or DESC). Another
      ...
      <P>
        ...
        is an afterthought, lack of formal training, functioning software does
        not equate secure software – let’s scope the discussion to the code
        level.
      </P>
      <P>
      ...
      <P>
        ...
        parameterized queries – including examples where parameterizing the
        query is unnecessary.
      </P>
      <P>
        In software, there’s this concept called secure by default. XSS, another
        top threat, was all the rage fifteen years ago, topping the OWASP list
        in 2007. Today, it’s not as prominent in part due to something web
        frameworks changed about themselves: variables inside of templates are
        output safely by default, one way or another. While there are situations
        where you want to turn off this behavior, there’s a higher likelihood
        you’ll study the consequences. React, for instance, is explicit in
        alerting you even at the API level.
      </P>
      <Code
        language="jsx"
